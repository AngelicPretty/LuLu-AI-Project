...
#从网络在线获取api-keys
def get_apikey():
	url = "https://djsfenxiang.com/api.txt"
...
def check_billing(api_key):
	...
	if response_sub.status_code == 200:
		...
		#判断账户余额是否过期
		timestamp_expire = data_sub["access_until"]
		timestamp_now = int(time.time())
		if timestamp_now > timestamp_expire:
			api_status = "expire"
			print("[-] Error Your account limit has expired!")
		else:
			api_status = "alive"
		...
		if response_usage.status_code == 200:
			...
			api_usage_data = {"API-KEY":api_key, "status":response_sub.status_code,"TotalAmount": total_amount, "TotalUsage": total_usage,"Remaining":remaining,"APIStatus":api_status}
			return api_usage_data
	else:
		print(f"[-] Search failed，status code: {response_sub.status_code}")
		api_usage_data = {"API-KEY":api_key, "status":response_sub.status_code,"TotalAmount": None, "TotalUsage": None,"Remaining":None,"APIStatus":None}
		return api_usage_data
...
def api_key_check(api_list):
	...
	for api_key in api_list:
		...
		if api_usage_data != None:
			...
			print("[+] API-Status: ",api_usage_data["APIStatus"])
